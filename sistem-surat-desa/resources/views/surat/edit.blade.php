        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('surat.update', $jenisSurat->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="kode_surat" class="block mt-1 w-full" type="text" name="kode_surat" :value="old('kode_surat', $jenisSurat->kode_surat)" required />
                                <x-input-error :messages="$errors->get('kode_surat')" class="mt-2" />
                            </div>
                            
                            <div>
                                <x-input-label for="nama_surat" :value="__('Nama Surat')" />
                                <x-text-input id="nama_surat" class="block mt-1 w-full" type="text" name="nama_surat" :value="old('nama_surat', $jenisSurat->nama_surat)" required />
                                <x-input-error :messages="$errors->get('nama_surat')" class="mt-2" />
                            </div>
                            
                            <div class="md:col-span-2">
                                <x-input-label for="template_path" :value="__('Template Path')" />
                                <x-text-input id="template_path" class="block mt-1 w-full" type="text" name="template_path" :value="old('template_path', $jenisSurat->template_path)" required />
